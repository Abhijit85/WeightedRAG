...
try:  # pragma: no cover
    from .tapas_table_extractor import TAPASTableExtractor, TAPASTable
except Exception:  # pragma: no cover
    TAPASTableExtractor = None  # type: ignore
    TAPASTable = None  # type: ignore
...
class TableProcessor:
    ...
    def __init__(self, timeout_seconds=10, tapas_model: Optional[str] = None):
        self.processed_count = 0
        self.timeout_seconds = timeout_seconds
        self.tapas_extractor: Optional[TAPASTableExtractor] = None
        tapas_name = tapas_model or os.getenv("TAPAS_MODEL_NAME") or "google/tapas-large-finetuned-wtq"
        if TAPASTableExtractor is not None:
            try:
                self.tapas_extractor = TAPASTableExtractor(model_name=tapas_name)
            except Exception as exc:
                print(f"⚠️  TAPAS extractor unavailable ({exc}). Falling back to BeautifulSoup parsing.")
    ...
    def process_table_entry(self, entry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            tapas_table = self._extract_with_tapas(entry['table_text'])
            ...
            processed = {
                ...
                'linearized_text': self._linearize_table_safe(entry['table_text'], tapas_table),
                'structured_data': self._parse_table_structure_safe(entry['table_text'], tapas_table),
                'natural_description': self._generate_natural_description_safe(
                    entry['table_text'], entry['question'], tapas_table
                ),
    ...
    def _extract_with_tapas(self, html_table: str) -> Optional["TAPASTable"]:
        if not self.tapas_extractor:
            return None
        try:
            return self.tapas_extractor.extract(html_table)
        except Exception:
            return None
    
    def _linearize_table_safe(self, html_table: str, tapas_table: Optional["TAPASTable"] = None) -> str:
        """Safe table linearization with TAPAS fallback."""
        if tapas_table:
            return tapas_table.linearized
        try:
    ...
    def _parse_table_structure_safe(self, html_table: str, tapas_table: Optional["TAPASTable"] = None) -> Dict[str, Any]:
        """Safe table structure parsing"""
        if tapas_table:
            return {
                'type': 'table',
                'headers': tapas_table.headers,
                'rows': tapas_table.rows,
                'num_rows': len(tapas_table.rows),
                'num_cols': len(tapas_table.headers),
                'source': 'tapas',
                'tapas_model': tapas_table.model_name
            }
        try:
            soup = BeautifulSoup(html_table, 'html.parser')
            table = soup.find('table')
    ...
    def _generate_natural_description_safe(self, html_table: str, question: str, tapas_table: Optional["TAPASTable"] = None) -> str:
        """Safe natural description generation"""
        try:
            structured = self._parse_table_structure_safe(html_table, tapas_table)
